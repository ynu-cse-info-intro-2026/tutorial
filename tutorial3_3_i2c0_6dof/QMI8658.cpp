...
#define QMI8658_printf Serial.printf
...
unsigned char QMI8658_write_reg(unsigned char reg, unsigned char value) {
  DEV_I2C_Write_Reg_Byte(SENSOR_I2C_PORT, QMI8658_slave_addr, reg, value);
...
unsigned char QMI8658_write_regs(unsigned char reg, unsigned char *value, unsigned char len) {
  ...
  for (i = 0; i < len; i++) {
   QMI8658_write_reg(reg + i, value[i]);
  }
...
unsigned char QMI8658_read_reg(unsigned char reg, unsigned char *buf, unsigned short len) {
  DEV_I2C_Read_Reg_nByte(SENSOR_I2C_PORT, QMI8658_slave_addr, reg, buf, len);
  return 0;
}
...
void QMI8658_read_ae(float quat[4], float velocity[3]) {
  ...
  velocity[0] = (float)(raw_v_xyz[0]) / ae_v_lsb_div;
  velocity[1] = (float)(raw_v_xyz[1]) / ae_v_lsb_div;
  velocity[2] = (float)(raw_v_xyz[2]) / ae_v_lsb_div;
}
